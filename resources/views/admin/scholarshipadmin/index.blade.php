@extends('admin.scholarshipadmin.master')
@section('content')
...
<div class="main-admin-adv-filter-container">
  ...
  <div class="row"> @include('admin.scholarshipadmin.leftpanel')
    <div class="column small-12 large-9 small-text-center large-text-left maintabcont" id="add" @if((count($scholarships)> 0) && ($page_type =='')) style="display:none;" @endif>
      <div class="tab"> <a class="tablinks active" onclick="openTab(event, '1')">Scholarship Information</a> <a class="tablinks" <?php if(isset($scholarship_info->id)){ ?> onclick="openTab(event, '2')" <?php }?> id="targetid"><span class="tooltip_scholarship">Targeting<?php if(!isset($scholarship_info->id)){ ?><span class="tooltiptext">You need to save the scholarship before adding targeting</span><?php }?></span></a> </div>
      <br />
      <br />
    ...
    <div class="column small-12 large-9 small-text-center large-text-left maintabcont" id="list" @if(count($scholarships)== 0 || $page_type!='') style="display:none;" @endif>
          ...
          <div class="sch-table-result-wrapper" id="scholDiv_{{$sch->id}}">
            <div class="sch-table-result clearfix">
              ...
              <div class="sch-col sch-col-add">
                <div class="sch-due"><a href="/scholarshipadmin/edit/{{$sch->id}}">Edit</a> | <a href="javascript:void(0);" onclick="DeltRecord({{$sch->id}})">Delete</a></div>
              </div>
            </div>
          </div>
          @endforeach
...
<script>
	...
	function openmainTab(evt, cName){
	 ...
	 $(document).on('click', '#scholarshipAdmBtn', function(){
		...
		$.ajax({
            url: '/scholarshipadmin/addScholarshipAdmin',
            type: 'POST',
            data: input,
            ...
            success: function(data){
				...
				if(data.success == "success" && data.scholarship_id!=0){
					if(data.operation == "add"){
						window.location = "/scholarshipadmin/edit/"+data.scholarship_id;
	...
	function DeltRecord(schid){
		...
		if($suretdel == true){
			$.ajax({
            	url: '/scholarshipadmin/delScholarshipAdmin',
            	type: 'POST',
            	data: {'id' : schid},
            	headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, 
           	 	success: function(data){
