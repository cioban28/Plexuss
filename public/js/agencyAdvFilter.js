// agencyAdvFilter.js
Plex.agencyAdvFilter = {
	currentTab: '',
	prevTab: '',
	currentTab_text: '',
	filter_section_route: 'filter/',
	save_filter_route: '/agency/ajax/setRecommendationFilter/',
	reset_filter_route: '/agency/ajax/resetRecommendationFilter/',
	getRecommendationPercent_route: '/agency/ajax/getNumberOfUsersForFilter',
...
$(document).ready(function(){
	Plex.agencyAdvFilter.initSections();
	if( Plex.agencyAdvFilter.isFilterLocked() ) Plex.agencyAdvFilter.shutdownFilters();
	Plex.agencyAdvFilter.showAllDoneBtn();
});
...
****************************************************************/
Plex.agencyAdvFilter.showAllDoneBtn = function(){
...
Plex.agencyAdvFilter.isFilterLocked = function(){
	return !!parseInt($('ul.adv-filtering-menu').first().data('locked'));
}

Plex.agencyAdvFilter.shutdownFilters = function(){
	_.each(Plex.agencyAdvFilter.sections.list, function(obj, index, arr){
...
Plex.agencyAdvFilter.initSections = function(){
	var sections = $('.adv-filtering-menu-container li[data-filter-tab]'), tab = null;

	Plex.agencyAdvFilter.sections = new SectionList();

	$.each(sections, function(val, index){
		tab = $(this).data('filter-tab');
		Plex.agencyAdvFilter.sections.addSection( new Section(tab) );
	});
}
...
Section.prototype.hasChanged = function(){
	this.changed = !0;
	Plex.agencyAdvFilter.sections.updateView();
};
...
Section.prototype.removeDiscardedCrumbs = function(){
	...
	Plex.agencyAdvFilter.sections.renderCrumbs();
}
...
Section.prototype.buildDeptUI = function(id){
	...
	if( ui ){
		...
		if( id ){
			...
			Plex.agencyAdvFilter.populateMajorBasedOnDepartment( id, dept_obj.value );
		}
	}
};
...
Plex.agencyAdvFilter.locationMajorBuild = function(crumbs){
...
Plex.agencyAdvFilter.selectFieldOnly = function(crumbs){
	var html = '';
...
SectionList.prototype.locationCrumbUI = Plex.agencyAdvFilter.locationMajorBuild;
SectionList.prototype.majorCrumbUI = Plex.agencyAdvFilter.locationMajorBuild;
SectionList.prototype.militaryAffiliationCrumbUI = Plex.agencyAdvFilter.selectFieldOnly;
SectionList.prototype.startDateTermCrumbUI = Plex.agencyAdvFilter.selectFieldOnly;
//SectionList.prototype.financialCrumbUI = Plex.agencyAdvFilter.selectFieldOnly;
SectionList.prototype.typeofschoolCrumbUI = Plex.agencyAdvFilter.selectFieldOnly;
...
Plex.agencyAdvFilter.uploadsEduDegreeBuild = function(crumbs){
	var html = '';
...
SectionList.prototype.uploadsCrumbUI = Plex.agencyAdvFilter.uploadsEduDegreeBuild;
SectionList.prototype.educationLevelCrumbUI = Plex.agencyAdvFilter.uploadsEduDegreeBuild;
SectionList.prototype.desiredDegreeCrumbUI = Plex.agencyAdvFilter.uploadsEduDegreeBuild;
...
SectionList.prototype.initCrumbs = function(section_name){
	...
	if( Plex.agencyAdvFilter.prevTab ){
		prevSection = this.getSection(Plex.agencyAdvFilter.prevTab);
		//if user discarded changes, remove those crumbs before adding more default ones
		if( prevSection ){
			prevSection.removeCrumbBySection(Plex.agencyAdvFilter.prevTab);
			Plex.agencyAdvFilter.sections.renderCrumbs();
		}
	}
	...
	//crumb init start
	Plex.agencyAdvFilter.isInit = true;
	...
	//crumb init end
	Plex.agencyAdvFilter.isInit = false;
}
...
Plex.agencyAdvFilter.initCheckboxes = function(elem_arr){
...
SectionList.prototype.inituploads = Plex.agencyAdvFilter.initCheckboxes;
SectionList.prototype.initeducationLevel = Plex.agencyAdvFilter.initCheckboxes;
SectionList.prototype.initdesiredDegree = Plex.agencyAdvFilter.initCheckboxes;
SectionList.prototype.initscores = Plex.agencyAdvFilter.initCheckboxes;
Plex.agencyAdvFilter.initLocMajorEthRel = function(elem_arr, section_name){
...
SectionList.prototype.initlocation = Plex.agencyAdvFilter.initLocMajorEthRel;
...
SectionList.prototype.initmajor = function(elem_arr, section_name){
	...
	Plex.agencyAdvFilter.majorsInitData = dept_arr;
...
Plex.agencyAdvFilter.resetChanges = function(tab){
	var section = null;
	section = Plex.agencyAdvFilter.sections.getSection(tab);
...
$(document).on('click', '.adv-filtering-menu li', function(e){
	...
	Plex.agencyAdvFilter.prevTab = Plex.agencyAdvFilter.currentTab;
	Plex.agencyAdvFilter.currentTab = $(this).data('filter-tab');

	section = Plex.agencyAdvFilter.sections.getSection(Plex.agencyAdvFilter.currentTab);
	if( section && section.locked ) return false;

	$('.adv-filtering-menu li').removeClass('active');

	if( Plex.agencyAdvFilter.change_has_been_made ){
		Plex.agencyAdvFilter.openDialogModal();
	}else{
		//no change has been made, so go ahead and just get select section	
		Plex.agencyAdvFilter.getSelectedFilterSection();
	}
...
Plex.agencyAdvFilter.getSelectedFilterSection = function(){
	var this_sections_fields = null, route = '';

	Plex.agencyAdvFilter.showLoader();

	if( Plex.agencyAdvFilter.currentTab === 'major' ) route = Plex.agencyAdvFilter.filter_section_route + 'majorDeptDegree';
	else route = Plex.agencyAdvFilter.filter_section_route + Plex.agencyAdvFilter.currentTab;
	...
	.done(function(data){
		...
		$('.adv-filtering-menu li[data-filter-tab="'+Plex.agencyAdvFilter.currentTab+'"]').addClass('active');
		$('.filtering-menu-sm-container').slideUp(250);

		//for mobile, update current section indicator
		Plex.agencyAdvFilter.currentTab_text = $('.adv-filtering-menu li[data-filter-tab="'+Plex.agencyAdvFilter.currentTab+'"]').first().text();
		$('.filter-page-indicator').html(Plex.agencyAdvFilter.currentTab_text);

		//show only when a tab is clicked and once its open, no need to keep calling .show()
		if( !Plex.agencyAdvFilter.save_reset_row_visible ){
			Plex.agencyAdvFilter.save_reset_row_visible = true;
		...
		Plex.agencyAdvFilter.filter_tags_array = [];
		Plex.agencyAdvFilter.checkIfTagsExistOnLoad();

		//save current state of this section's form fields
		this_sections_fields = $('.filter-by-'+Plex.agencyAdvFilter.currentTab+'-container').find('.filter-this');

		//reset change_has_been_made to false when opening new filter section
		Plex.agencyAdvFilter.change_has_been_made = false;

		//save current forms fields
		Plex.agencyAdvFilter.currentFormFields = this_sections_fields;

		//only for location filter - hide select content if filter by ALL is checked on load
		Plex.agencyAdvFilter.hideRadioBtnContentOnload();

		//if country section loads with US selected then show state/city, otherwise hide
		if( Plex.agencyAdvFilter.currentTab === 'location' ){
			Plex.agencyAdvFilter.countryOnLoad();
		}

		Plex.agencyAdvFilter.sections.initCrumbs(Plex.agencyAdvFilter.currentTab);

		Plex.agencyAdvFilter.hideLoader();
	});
}

Plex.agencyAdvFilter.getVideoSection = function(){
	var this_sections_fields = null, route = '';

	Plex.agencyAdvFilter.showLoader();
	
	route = Plex.agencyAdvFilter.filter_section_route + Plex.agencyAdvFilter.currentTab;
...
Plex.agencyAdvFilter.hideRadioBtnContentOnload = function(){
	if( Plex.agencyAdvFilter.currentTab === 'location' || Plex.agencyAdvFilter.currentTab === 'major' ){

		$.each(Plex.agencyAdvFilter.currentFormFields, function(){
			var _this = $(this);
...
$(document).on('click', '.targeting-video', function(){
	Plex.agencyAdvFilter.currentTab = 'video';

	Plex.agencyAdvFilter.getVideoSection();
...
$(document).on('change', '.filter-page-section .filter-this', function(){
	...
	if( !Plex.agencyAdvFilter.isInit ){
		Plex.agencyAdvFilter.change_has_been_made = true;
		section.hasChanged();
	}
	...
	if( alreadyHaveCrumb.length > 0 ){
		Plex.agencyAdvFilter.editCrumb(_this, alreadyHaveCrumb);
	}else{
		Plex.agencyAdvFilter.createCrumb(_this);	
	}
...
Plex.agencyAdvFilter.createCrumb = function(elem){
	...
	section_obj = Plex.agencyAdvFilter.sections.getSection(section_name);
	...
	if( !skip ){
		...
		if(section_name === 'financial' && option != 'Interested in aid'){
			...
			Plex.agencyAdvFilter.setFinancialCrumbList(component, option, section_obj, val, min, max);
		}
		else{ //everything else - just add one crumb
			crumb = new Crumb(section_name, component, option, val, min, max, elem_id, childOf);


			if( !Plex.agencyAdvFilter.isInit ) crumb.edit('isDefault', false);
...
Plex.agencyAdvFilter.editCrumb = function(elem, old_crumbs){
	var section_name = elem.closest('.filter-page-section').data('section'),
		component = elem.closest('.component').data('component'),
	...
	if( !isDuplicate) Plex.agencyAdvFilter.sections.renderCrumbs();
}



Plex.agencyAdvFilter.removeCrumb = function(elem){
	...
	switch(section_name){
		case 'location':
			...
			Plex.agencyAdvFilter.removeTag(val);
			if( crumb.length === 1 ){
...
$(document).on('click', '.tag .remove', function(){
	...
	Plex.agencyAdvFilter.removeCrumb(_this);
});
...
*********************************************************************/
Plex.agencyAdvFilter.setFinancialCrumbList = function(component, option, section_obj, val, min ,max){
						...
						for(var i = $.inArray(val.trim(), Plex.agencyAdvFilter.ranges) + 1; i < Plex.agencyAdvFilter.ranges.length; i++){
							
							var ncrumb = new Crumb('financial', component, Plex.agencyAdvFilter.ranges_formatted[i], Plex.agencyAdvFilter.ranges[i], min, max);
...
//open dialog modal
Plex.agencyAdvFilter.openDialogModal = function(){
	$('#filter-dialog-modal').foundation('reveal', 'open');
}

//closes dialog modal
Plex.agencyAdvFilter.closeDialogModal = function(){
...
$(document).on('click', '#filter-dialog-modal .save', function(){
	var filterData = null, clicked_section = $('.adv-filtering-menu li[data-filter-tab="'+Plex.agencyAdvFilter.currentTab+'"]').first();

	Plex.agencyAdvFilter.currentTab = Plex.agencyAdvFilter.prevTab;
	filterData = Plex.agencyAdvFilter.buildJSONDataOfValuesToPost(Plex.agencyAdvFilter.currentFormFields);	
	Plex.agencyAdvFilter.closeDialogModal();
	Plex.agencyAdvFilter.saveFilter(filterData, clicked_section);	
});

//closes modal, does NOT save, and changes filter section
$(document).on('click', '#filter-dialog-modal .discard', function(){
	Plex.agencyAdvFilter.resetChanges(Plex.agencyAdvFilter.prevTab);
	Plex.agencyAdvFilter.getSelectedFilterSection();
	Plex.agencyAdvFilter.closeDialogModal();
});
...
Plex.agencyAdvFilter.resetFields = function(){
	var section = null;

	switch(Plex.agencyAdvFilter.currentTab){
		case 'location':
			$('#all_country_filter').prop('checked', true).trigger('change');
			section = Plex.agencyAdvFilter.sections.getSection(Plex.agencyAdvFilter.currentTab);
			section.removeCrumbBySection(Plex.agencyAdvFilter.currentTab);
			Plex.agencyAdvFilter.sections.renderCrumbs();
			break;
		case 'startDateTerm': 
			$('#startDateTerm_filter').val('').trigger('change');
			Plex.agencyAdvFilter.eraseAndRenderCurrentSection();
			break;
		case 'financial':
		...
		case 'typeofschool':
			...
			break;
		case 'major':
			Plex.agencyAdvFilter.eraseMajors();
			break;
		case 'scores':
			Plex.agencyAdvFilter.eraseScores();
			break;
		case 'uploads':
			Plex.agencyAdvFilter.eraseUploads();
			break;
		case 'demographic':
			Plex.agencyAdvFilter.eraseDemo();
			break;
		case 'educationLevel':
			Plex.agencyAdvFilter.eraseEdu();
			break;
		case 'desiredDegree':
			Plex.agencyAdvFilter.eraseDegree();
			break;
		case 'militaryAffiliation':
			Plex.agencyAdvFilter.eraseMilitary();
			break;
		case 'profileCompletion':
			Plex.agencyAdvFilter.eraseProfile();
			break;
	}

	return Plex.agencyAdvFilter.currentFormFields;
};

Plex.agencyAdvFilter.eraseBySection = function(){
	var section = null;
	section = Plex.agencyAdvFilter.sections.getSection(Plex.agencyAdvFilter.currentTab);
	section.removeCrumbBySection(Plex.agencyAdvFilter.currentTab);
	Plex.agencyAdvFilter.sections.renderCrumbs();
...
Plex.agencyAdvFilter.eraseMajors = function(){
	$('#all_department_filter').prop('checked', true).trigger('change');
	Plex.agencyAdvFilter.eraseBySection();
};

Plex.agencyAdvFilter.eraseProfile = function(){
	$('#profileCompletion_filter').val('');
	Plex.agencyAdvFilter.eraseBySection();
};

Plex.agencyAdvFilter.eraseMilitary = function(){
	$('#inMilitary_filter').val('').trigger('change');
	Plex.agencyAdvFilter.eraseBySection();
};

Plex.agencyAdvFilter.eraseDegree = function(){
	Plex.agencyAdvFilter.eraseBySection();
	$('.checkbox-filter.filter-this').prop('checked', true).trigger('change');
};

Plex.agencyAdvFilter.eraseEdu = function(){
	Plex.agencyAdvFilter.eraseBySection();
	$('#hsUsers_filter, #collegeUsers_filter').prop('checked', true).trigger('change');
};

Plex.agencyAdvFilter.eraseDemo = function(){
...
Plex.agencyAdvFilter.eraseUploads = function(){
...
Plex.agencyAdvFilter.eraseScores = function(){
...
Plex.agencyAdvFilter.eraseAndRenderCurrentSection = function(){
	var section = null;

	section = Plex.agencyAdvFilter.sections.getSection(Plex.agencyAdvFilter.currentTab);
	section.removeCrumbByComponent(Plex.agencyAdvFilter.currentTab);
	Plex.agencyAdvFilter.sections.renderCrumbs();
};

$(document).on('click', '.reset-filters-btn', function(e){
	Plex.agencyAdvFilter.resetFilter();
});

// ----------------- COME BACK TO RESET FILTER -----------------
Plex.agencyAdvFilter.resetFilter = function(){
	var filterData = null, formFields = [], section = null;

	// if( confirm('Are you sure you want to reset ' + Plex.agencyAdvFilter.currentTab.toUpperCase() + ' filter back to default? Click "OK" to confirm.') ){
	// 	formFields = Plex.agencyAdvFilter.resetFields(),
	// 	Plex.agencyAdvFilter.resetAll();
	// }
...
Plex.agencyAdvFilter.resetAll = function(){
	var route = '';

	Plex.agencyAdvFilter.showLoader();

	if( Plex.agencyAdvFilter.currentTab === 'major' ) route = Plex.agencyAdvFilter.reset_filter_route + 'majorDeptDegree';
	else route = Plex.agencyAdvFilter.reset_filter_route + Plex.agencyAdvFilter.currentTab;
	...
	$.ajax({
		...
		success: function(data){
			if( data === 'done' ){
				Plex.agencyAdvFilter.successfullyResetFilterAlert();
				Plex.agencyAdvFilter.getNumberOfUsersForFilter(); //adjust filter percentage
			}
			else Plex.agencyAdvFilter.erroredResetFilterAlert();

			Plex.agencyAdvFilter.change_has_been_made = false;
			Plex.agencyAdvFilter.hideLoader();
		},
		error: function(err){
			Plex.agencyAdvFilter.erroredResetFilterAlert();
		}
	});
};
...
$(document).on('click', '.save-filters-btn', function(e){
	...
	passes_validation = Plex.agencyAdvFilter.selectFieldValidation(this_forms_fields);

	if( passes_validation ){
		$(this).closest('.common-container-for-filter-sections').find('.select-option-error').slideUp(250);

		if( Plex.agencyAdvFilter.currentTab === 'major' ) filter_data = Plex.agencyAdvFilter.buildCustomMajorData();
		else filter_data = Plex.agencyAdvFilter.buildJSONDataOfValuesToPost(this_forms_fields);

		Plex.agencyAdvFilter.saveFilter(filter_data);
...
//saving filter to db
Plex.agencyAdvFilter.saveFilter = function(filter_data, fromDiaglog){
	var saveFilter_route = Plex.agencyAdvFilter.save_filter_route + Plex.agencyAdvFilter.currentTab,
		section_obj = Plex.agencyAdvFilter.sections.getSection(Plex.agencyAdvFilter.currentTab);

	Plex.agencyAdvFilter.showLoader();

	if( Plex.agencyAdvFilter.currentTab === 'major' ) saveFilter_route = Plex.agencyAdvFilter.save_filter_route + 'majorDeptDegree';
	...
	$.ajax({
		...
		Plex.agencyAdvFilter.successfullySavedFilterAlert();
		Plex.agencyAdvFilter.change_has_been_made = false;
		Plex.agencyAdvFilter.getNumberOfUsersForFilter();
		section_obj.initStatus(!1);

		if( fromDiaglog ) fromDiaglog.trigger('click');
		else Plex.agencyAdvFilter.hideLoader();
	});
}

Plex.agencyAdvFilter.buildCustomMajorData = function(){
...
Plex.agencyAdvFilter.showLoader = function(){
	$('.targeting-ajax-loader').show();
};
Plex.agencyAdvFilter.hideLoader = function(){
	$('.targeting-ajax-loader').hide();
};

//gets the percentage of user recommended based on filter changes
Plex.agencyAdvFilter.getNumberOfUsersForFilter = function(){
	$.ajax({
		url: Plex.agencyAdvFilter.getRecommendationPercent_route,
		type: 'GET',
		headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
...
Plex.agencyAdvFilter.successfullySavedFilterAlert = function(){
	var alert_msg = {
        textColor: '#fff',
        backGroundColor : '#26b24b',
        msg: Plex.agencyAdvFilter.currentTab_text + ' filter has been successfully saved! <br/> Note:  Your targeting changes may impact some students.  As a result you may not be able to find them on the current portal.  You can always find the student in the general portal.',
        type: 'soft',
        dur : 10000
    };	
...
//successfully saved filter alert
Plex.agencyAdvFilter.successfullyResetFilterAlert = function(){
	var alert_msg = {
        textColor: '#fff',
        backGroundColor : '#26b24b',
        msg: Plex.agencyAdvFilter.currentTab_text + ' filter has been successfully reset!',
        type: 'soft',
        dur : 5000
    };
...
//successfully saved filter alert
Plex.agencyAdvFilter.erroredResetFilterAlert = function(){
	var alert_msg = {
        textColor: '#fff',
...
Plex.agencyAdvFilter.buildJSONDataOfValuesToPost = function(form_fields){
	...
	$.each(form_fields, function(){
		...
		if( _this.hasClass('select-filter') ){
			...
			if( Plex.agencyAdvFilter.currentTab === 'militaryAffiliation' ){
				crumbs = null;
				if( _this.hasClass('inMili') ){
					arr = Plex.agencyAdvFilter.buildSelectFilterArrayWithCrumbs('inMilitary');
					arr = arr[0];
				}else{
					arr = Plex.agencyAdvFilter.buildSelectFilterArrayWithCrumbs('militaryAffiliation');
				}
				values_to_send[tmp_key] = arr;
			}else if( Plex.agencyAdvFilter.currentTab === 'profileCompletion' ){
				values_to_send[tmp_key] = _this.val();
			}else if( Plex.agencyAdvFilter.currentTab === 'startDateTerm' || Plex.agencyAdvFilter.currentTab === 'financial' ){
				values_to_send[tmp_key] = Plex.agencyAdvFilter.buildSelectFilterArrayWithCrumbs(Plex.agencyAdvFilter.currentTab);
			}else{
				values_to_send[tmp_key] = Plex.agencyAdvFilter.buildSelectFilterArray(_this.attr('name'));
			}
...
Plex.agencyAdvFilter.buildSelectFilterArrayWithCrumbs = function(comp){
...
Plex.agencyAdvFilter.buildSelectFilterArray = function(this_type){
	var tmp_array = [];

	tmp_array = _.chain(Plex.agencyAdvFilter.filter_tags_array)
...
Plex.agencyAdvFilter.selectFieldValidation = function(form_fields){
	var is_valid = true;
		...
		if( _this.hasClass('radio-filter') ){
			...
			if( _this.is(':checked') ){
				if( _this.val() !== 'all' && (_this.val() === 'include' || _this.val() === 'exclude' ) ){
					if( _.isEmpty(_.filter(Plex.agencyAdvFilter.filter_tags_array, {type: _this.attr('name')})) ){
						is_valid = false;
						return false;
					}
				}
	...
	if( is_valid && (Plex.agencyAdvFilter.currentTab === 'scores' || Plex.agencyAdvFilter.currentTab === 'demographic') ){
		if( Plex.agencyAdvFilter.minMaxComparison() ){
	...
	if( is_valid && Plex.agencyAdvFilter.currentTab == 'educationLevel' ){
		if(!Plex.agencyAdvFilter.atleastOneCheckboxChecked(Plex.agencyAdvFilter.currentFormFields)){
			is_valid = false;
		}
	}
	...
	if( Plex.agencyAdvFilter.currentTab === 'major' ) is_valid = true;

	Plex.agencyAdvFilter.showHideErrors(is_valid);	
...
Plex.agencyAdvFilter.showHideErrors = function(is_valid){
	if( !is_valid ){
...
Plex.agencyAdvFilter.atleastOneCheckboxChecked = function(checkboxes){
...
Plex.agencyAdvFilter.minMaxComparison = function(){

	//calls a function that organizes input values into an array of objects
	var minMax_object_list = Plex.agencyAdvFilter.organizedMinMaxValues();
...
Plex.agencyAdvFilter.organizedMinMaxValues = function(){
	var minMax_array = [];
	tmp_min = '';
	tmp_obj = null;
...
$(document).on('change', '.adv-filtering-section-container select', function(){
	...
	if( this_text !== 'Select..' && this_value !== '' ){
		...
		if( !duplicate_found ){
			Plex.agencyAdvFilter.saveTag(this_text, this_value, this_type);
			if( this_type === 'department' ){
				section.buildDeptUI(id);
			}
		}

		Plex.agencyAdvFilter.addTagToCorrectViewContainer($(this), this_type);
	}
});
...
$(document).on('click', '.show-major', function(){
	...
	if( _this.hasClass('needs-init') ){
		Plex.agencyAdvFilter.sections.initMajorBuild(dept);
...
$(document).on('change', '#state_filter', function(){
    ...
    if(_this.val() != ''){
       Plex.agencyAdvFilter.populateCityBasedOnState(_this.val());
    }
});

//save tags in global array
Plex.agencyAdvFilter.saveTag = function(text, val, type){
	var tag = {};
	tag.name = text;
	tag.val = val;
	tag.type = type;
	Plex.agencyAdvFilter.filter_tags_array.push(tag);
}


//remove tags from global array
Plex.agencyAdvFilter.removeTag = function(tag_id){
	Plex.agencyAdvFilter.filter_tags_array = _.reject(Plex.agencyAdvFilter.filter_tags_array, {val: tag_id});
}


//remove multiple tags by type
Plex.agencyAdvFilter.removeMultipleTags = function(tag_type){
	Plex.agencyAdvFilter.filter_tags_array = _.reject(Plex.agencyAdvFilter.filter_tags_array, {type: tag_type});
}


//no duplicate tags
Plex.agencyAdvFilter.noDuplicateObjects = function(obj_array, obj_val, obj_type){
	var result = null;
...
Plex.agencyAdvFilter.addTagToCorrectViewContainer = function(elem, this_type){
	var tmp_arr = _.filter(Plex.agencyAdvFilter.filter_tags_array, {type: this_type});
	$(elem).closest('.contains-tags-row').find('.filter-tags-list').html( Plex.agencyAdvFilter.buildFilterTags(tmp_arr) );	
}


//build tag html to display on the page
Plex.agencyAdvFilter.buildFilterTags = function(tag_list){
	var html_tags = '';
...
Plex.agencyAdvFilter.checkIfTagsExistOnLoad = function(){
	...
	if( tags_lists_found.length > 0 ){
		if( tags_exist_onLoad.length > 0 ){
			...
			$.each(tags, function(){
				...
				Plex.agencyAdvFilter.saveTag(tag_name, tag_name, tag_type);
			});
		}
	}
}
...
$(document).on('keypress', '.achievements-col input', function(){
	...
	if( _this.val() != '' && !has_created_new_field ){
		...
		new_field = Plex.agencyAdvFilter.buildNewInputField(field_name);
...
Plex.agencyAdvFilter.buildNewInputField = function(name){
	var input_field = '';
...
$(document).on('change', '.adv-filtering-section-container .radio-filter', function(){
	...
	if( _this.val() == 'all' ){
		...
		if( Plex.agencyAdvFilter.currentTab === 'location' && _this.attr('id') === 'all_state_filter' ){
			...
			Plex.agencyAdvFilter.removeMultipleTags('state');
			$('#all_city_filter').parent().find('select').val('').parent().find('.filter-tags-list').html('<small>No city selected yet.</small>');
			Plex.agencyAdvFilter.removeMultipleTags('city');

		}else if( Plex.agencyAdvFilter.currentTab === 'location' && _this.attr('id') === 'all_city_filter' ){
			_this.parent().find('select').val('').parent().find('.filter-tags-list').html('<small>No city selected yet.</small>');
			Plex.agencyAdvFilter.removeMultipleTags('city');

		}else if( Plex.agencyAdvFilter.currentTab === 'location' && _this.attr('id') === 'all_country_filter' ){
			...
			Plex.agencyAdvFilter.removeMultipleTags('country');
			Plex.agencyAdvFilter.toggleUS();

		}else if( Plex.agencyAdvFilter.currentTab === 'major' && _this.attr('id') === 'all_department_filter' ){
			section = Plex.agencyAdvFilter.sections.getSection('major');
		...
		}else if( Plex.agencyAdvFilter.currentTab === 'major' && _this.attr('id').indexOf('_major') > -1 ){
		...
		}else if( Plex.agencyAdvFilter.currentTab === 'demographic' && _this.attr('id') === 'all_eth_filter' ){
			_this.parent().find('select').val('').parent().find('.filter-tags-list').html('<small>No ethinicity selected yet.</small>');
			Plex.agencyAdvFilter.removeMultipleTags('ethnicity');
		}
...
Plex.agencyAdvFilter.toggleUS = function(){
	if( Plex.agencyAdvFilter.hasUS() )
		Plex.agencyAdvFilter.toggleCountrySection('open');
	else
		Plex.agencyAdvFilter.toggleCountrySection('close');
}

Plex.agencyAdvFilter.hasUS = function(){
	var us_found = null;
	us_found = _.findWhere(Plex.agencyAdvFilter.filter_tags_array, {type: 'country', val: 'United States'});
	return _.isObject(us_found);
}

Plex.agencyAdvFilter.toggleCountrySection = function(toggle){
	if(toggle === 'open')
...
Plex.agencyAdvFilter.countryOnLoad = function(){
	if( $('#all_country_filter').is(':checked') || !Plex.agencyAdvFilter.hasUS() ){
		Plex.agencyAdvFilter.toggleCountrySection('close');
	}
}

Plex.agencyAdvFilter.populateCityBasedOnState = function(stateAbbr){
...
Plex.agencyAdvFilter.populateMajorBasedOnDepartment = function(departmentId, dept){
