...
Plex.chat = {
	...
	topicReadUrl : '/agency/ajax/messages/setMsgRead/',
...
Plex.chat.switchToMessagesWithUser = function( thread_type, thread_id, recipient_id ){
	...
	Plex.chat.toggleChatAndMessageWindowsViews( 'agency-messages' );
}
/***********************************************************************/
...
Plex.chat.getTopicList = function ( callback ){
	...
	} else if( _this.windowActiveNow == 'agency-messages' || _this.windowActiveNow == 'agency-chat' ){
...
Plex.chat.addTopicList = function (data){
	...
	// $('#agency-chat > span.tooltip').remove();
...
Plex.chat.getMessages = function (){
	...
	.done(function(data) {
		_this.addmessages( data, isappend);
	});
}
/************************************************************************/
 ...
 */
Plex.chat.addmessages = function(data, isappend){
...
Plex.chat.checkMessagesView = function(){
	if (Plex.messages.windowActiveNow == 'agency-messages') {
...
/*$(document).on('mouseenter', '.prof-tooltip.has-tip', function(e){
	...
	$('#agency-chat .tooltip').is(':visible').show();
});*/

$(document).on('mouseout', '.prof-tooltip.has-tip', function(e){
	e.preventDefault();	
	$(this).addClass('open');
	$('#agency-chat .tooltip').hide();
});

// $(document).on('mouseenter', '#agency-chat .tooltip', function(){
