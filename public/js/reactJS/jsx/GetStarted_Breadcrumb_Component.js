...
var GetStarted_Breadcrumb_Component = React.createClass({displayName: "GetStarted_Breadcrumb_Component",
	...
	componentWillMount: function(){
		...
		for (var i = 1; i <= this.state.num_of_steps; i++) {
			...
			steps.push(obj);
			if( i <= this.state.max_steps ) stepBreadCrumbs.push(React.createElement(Step, {key: i, stepObj: obj}));
		}
	...
	initBadges: function(){
		...
		_.each(copy, function(obj, i){
			ui.push( React.createElement(ProfileBadges, {key: i, badge: obj}) );
		});
	...
	render: function(){
		...
		return (
			React.createElement("div", {className: "breadcrumb-container text-center"}, 
				 this.state.active_step.num === 8 ?
					React.createElement("div", {className: "congrats text-left"}, congrats) :
					React.createElement("div", {className: "breadcrumb-bar clearfix"}, 
						this.state.step_crumbs
					), 
				
				 this.state.badges_ui, 
				React.createElement(ProfileMeter, {perc: this.state.percentage})
			)
		);
	}
});
...
var Step = React.createClass({displayName: "Step",
	...
	render: function(){
		...
		return (
			React.createElement("div", {className: stepClass}, 
				React.createElement("a", {href: route, onClick: this.ableToRoute}, 
					React.createElement("div", {className: classes}), 
					React.createElement("div", {className: "step-num"}, "Step ", step.num, " ", React.createElement("span", {className: "show-for-small-only"}, " of ", step.total_num_of_steps))
				)
			)
		);
	}
...
var ProfileMeter = React.createClass({displayName: "ProfileMeter",
	componentDidMount: function(){
		this.findOffset();
	},
	...
	render: function(){
		return (
			React.createElement("div", {className: "meter-container hide-for-small-only"}, 
				React.createElement("div", {id: "cont", "data-pct": this.props.perc}, 
					React.createElement("svg", {id: "svg", width: "60", height: "60", viewPort: "0 0 100 100", version: "1.1", xmlns: "http://www.w3.org/2000/svg"}, 
						React.createElement("circle", {r: "21", cx: "30", cy: "30", fill: "transparent"}), 
						React.createElement("circle", {id: "bar", r: "21", cx: "30", cy: "30", fill: "transparent"})
					)
				)
			)
		);
...
var ProfileBadges = React.createClass({displayName: "ProfileBadges",
	render: function(){
		var badge = this.props.badge, badgeClass = 'bimg badge ' + badge.name;

		return (
			React.createElement("div", {className: "profile-badges hide-for-small-only"}, 
				React.createElement("div", {className: badgeClass}, 
					 badge.locked ? React.createElement("div", {className: "bimg locked"}) : null
				)	
			)
		);
	}
