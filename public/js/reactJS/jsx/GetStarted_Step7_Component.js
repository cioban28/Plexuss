...
var GetStarted_Step7_Component = React.createClass({displayName: "GetStarted_Step7_Component",
		...
		for (var i = 0; i < copy.length; i++) {
			...
			if( copy[i].section.indexOf('Specialty Schools') > -1 ){
				if( copy[i].section === 'Specialty Schools - ' + activeTab.name ){
					...
					ui.push( React.createElement(College_Carousel, {key: i, name: copy[i].section, route: this.state.get_route, 
											hasOptions: options, tabs: tab, changeTab: this.changeTab}) );
				}
			}else ui.push( React.createElement(College_Carousel, {key: i, name: copy[i].section, route: this.state.get_route}) );
		}
	...
	render: function(){
		return (
			React.createElement("div", {className: "step-container step6-head"}, 
				React.createElement("div", {className: "row"}, 
					React.createElement("div", {className: "column small-12 medium-7 large-6 medium-centered"}, 
						React.createElement("h3", null, 'Congrats, you can now be recruited by colleges!')
					), 
					React.createElement("div", {className: "column small-12 medium-3 medium-offset-9 text-right hide-for-small-only"}, 
				...
				React.createElement("div", {className: "row"}, 
					React.createElement("div", {className: "column small-12"}, 
						React.createElement("div", {className: "page-nav text-center"}, 
							React.createElement("div", null, React.createElement("a", {href: this.state.back_route}, "Back")), 
							React.createElement("div", {onClick: this.save}, "Continue"), 
							React.createElement("div", null, React.createElement("a", {href: "", onClick: this.save}, "Skip"))
						)
					)
				), 
...
var College_Carousel = React.createClass({displayName: "College_Carousel",
	getInitialState: function(){
		return {
			owlItems: [],
			inquiried: false,
	...
	initOwlItems: function(){
		...
		_.each(schools, function(obj){
			img = obj.logo_url || 'default-missing-college-logo.png';
			url = 'https://s3-us-west-2.amazonaws.com/asset.plexuss.com/college/logos/'+img;
			items.push( React.createElement(OwlItem, {key: obj.id, logo: url, name: obj.school_name, school_id: obj.id}) );	
		});
	...
	render: function(){
		var owl_name = 'owl-carousel '+this.modName()+'_owl',
			header = React.createElement(BasicHeader, {name: this.props.name}),
			body = React.createElement(Small_Loader, {size: "sm"});

		if( this.props.hasOptions ) header = React.createElement(OptionsHeader, {name: this.props.name.split('-')[0], tabs: this.props.tabs, changeTab: this.props.changeTab});
		if( this.state.owlItems && this.state.owlItems.length > 0 ){
			body = React.createElement(Carousel, {owlItems: this.state.owlItems, 
							 owl_name: owl_name, 
...
var Carousel = React.createClass({displayName: "Carousel",
	...
	render: function(){	
		return (
			React.createElement("div", {className: "gs-carousel"}, 
				React.createElement(Arrow, {direction: "a-left"}), 
				React.createElement("div", {className: "owl-container"}, 
					React.createElement("div", {className: this.props.owl_name}, 
						this.props.owlItems	
					)
				), 
				React.createElement(Arrow, {direction: "a-right"})
			)
		);
	}
});

var OptionsHeader = React.createClass({displayName: "OptionsHeader",
	getInitialState: function(){
		return {
			tab_ui: null
		};
	...
	buildTabs: function(){
		...
		_.each(tabs, function(obj, i){
			...
			ui.push( React.createElement("div", {className: classes, key: i, onClick: p.changeTab}, obj.name) );
		});
	...
	render: function(){
		return(
			React.createElement("div", {className: "carousel-name w-options"}, 
...
var BasicHeader = React.createClass({displayName: "BasicHeader",
	render: function(){
		return (
			React.createElement("div", {className: "carousel-name"}, 
				React.createElement("div", null, this.props.name)
			)
		);
	}
...
var Arrow = React.createClass({displayName: "Arrow",
	...
	render: function(){
		...
		return (
			React.createElement("div", {className: classes, onClick: this.move}, 
				React.createElement("div", {className: "arrow"}), 
				React.createElement("div", {className: "arrow shadow"})
			)
		);
	}
});

var OwlItem = React.createClass({displayName: "OwlItem",
	getInitialState: function(){
		return {
			school: null 
		};
	...
	render: function(){
		...
		return(
			React.createElement("div", {className: "item text-center"}, 
				React.createElement("div", {className: "logo", style: img}), 
				React.createElement("div", {className: "name"}, p.name.substr(0, 20)+'...'), 
				React.createElement(Recruit_Btn, {inqd: this.state.school.inquiried, madeInq: this.madeInquirie, 
...
var Recruit_Btn = React.createClass({displayName: "Recruit_Btn",
	render: function(){
		return (
			React.createElement("div", null, 
				 this.props.inqd ? 
					[React.createElement("span", {className: "check", key: -1}, "✓")] : 
					React.createElement("a", {href: "", className: "recruit-me-pls", "data-id": this.props.school_id}, 
...
// }
	...
	ReactDOM.render( React.createElement(GetStarted_Step7_Component, null), document.getElementById('get_started_step7') );
