...
var GetStarted_Step5_Component = React.createClass({
	...
	render: function(){
		...
		return (
			<div className="step_container">
				<div className="row gpa_page">
					<div className="column small-12">
						<div className="row">
							<div className="columns small-12 medium-6 large-7">
								<div className="intro">Enter your GPA so far</div>
								<br />
								<form>
									...
									<input 
										...
										value="2.87" />
									<label className="no_exams" htmlFor="idk_gpa">I don't know my GPA</label>

									<br />
									<br />

									<div className={ knowsGPA ? 'all_gpa_container' : 'all_gpa_container hide' }>
										{ !is_us_student && <NumberInput 
																name="unweighted_gpa" 
										...
										{ is_us_student && <div>
																<NumberInput name="unweighted_gpa" isValid={this.makeSaveActive} req="true" 
																			max={this.state.u_max} val={unweighted} 
																			validated={this.state['unweighted_gpa_is_valid'] != null ? this.state['unweighted_gpa_is_valid'] : true} />
																<br />
																<NumberInput name="weighted_gpa" isValid={this.makeSaveActive} 
									...
									<input id="no_exams_taken" type="checkbox" name="no-exams-taken" onChange={this.toggleExams} checked={this.state.no_exams_taken} />
									<label className="no_exams" htmlFor="no_exams_taken">I have not taken any exams yet</label>
									<br />

									<CollegeExams scores={user || null} unchangedUserInfo={unchanged_user_info} examsTaken={this.state.no_exams_taken} isValid={this.formIsValid} makeSaveActive={this.makeSaveActive} />

									{ !this.state.is_valid && this.state.save_has_been_clicked ? 
										<div className="err">
											<div><small>Check for invalid entries</small></div>
											<div><small>Fields cannot be left empty</small></div>
											<div><small>{'Unweighted parameters: 0.01 - 4'}</small></div>
											<div><small>{'Weighted parameters: 0.01 - 5'}</small></div>
										</div> : null 
									}
									...
									<div className="clearfix">
										<div className="right hide-for-small-only"><small>{'*Required for recruitment'}</small></div>
										<div className="text-center show-for-small-only"><small>{'*Required for recruitment'}</small></div>
									</div>
								</form>
							</div>
							
							<div className="column small-12 medium-6 large-5 g-con">
								<div className="gpa-conv-btn hide-for-small-only">
									<img className="calc" src="https://s3-us-west-2.amazonaws.com/asset.plexuss.com/profile/calc_icon.png" />
									{ !showConverter && <div>Not from the United States?</div> }
									{ !showConverter && <div onClick={ this._toggleGPAConverter }><u>Use this GPA Converter</u></div> }
									{ showConverter && <div className="closex" onClick={ this._toggleGPAConverter }><u>Close</u></div> }
								</div>

								<div className={ showConverter ? '' : 'hide' }>
									<GPA_Conversion country={user ? (user.country_name || '') : ''} />
								</div>
							</div>
						</div>
					</div>
...
var NumberInput = React.createClass({
	...
	render: function(){
		...
		return (
			<div className="form-container gpa clearfix">
				<div className="f-label capitalize" style={_style}>
					<label>{label_name}</label>
				</div>
				<div className="f-input" style={_finputStyle}>
					<input name={this.props.name} type="number" min="0.01" max={this.props.max} step="0.01" 
							placeholder={placehldr} className={input_class} onChange={this.update}
							style={_inputStyle}
							value={this.state.valu} />
					{ !this.props.validated && <small>Only values between {0.01} and {this.props.max} accepted</small> }
				</div>
				{
					this.props.req ?
					<div className="f-label text-center">
						<label>{'*'}</label>
					</div> : null
				}
			</div>
...
var CollegeExams = React.createClass({
	getInitialState: function(){
		return {
			exams: [],
			active_exam: null,
	...
	buildExams: function(){
		...
		for (var i = 0; i < e.length; i++) {
			...
			btn_ui.push( <Exam_Button key={key} id={btn_id} name={btn_name} 
			...
			ui.push(<Exam key={i} exam={e[i].name} type={e[i].type} fields={e[i].fields} is_pre_2016={is_pre_2016 || 0}
	...
	render: function(){
		...
		return (
			<div className={containerClasses}>
				<div>College Entrance Exams</div>
				<div className="clearfix exam-options">
...
var Exam_Button = React.createClass({
	getInitialState: function(){
		return {
			validated: true
		}
	...
	render: function(){
		var classes = this.props.active ? 'left exam-btn active' : 'left exam-btn',
			name = this.props.changed ? [this.props.name, <span key={-1}>&#x02713;</span>] : this.props.name;

			classes += this.state.validated ? '' : ' invalid';

		return(
			<div id={this.props.id} className={classes} onClick={this.props.examChosen}>{name}</div>
		);
	}
});
...
var Exam = React.createClass({
	...
	getExamFields: function(){
		...
		if( is_old_exam ){
			eFields.push(
				<div>
					<input className={this.state.examValidated ? '' : 'invalid'} id={'pre_'+p.exam[0]} type="checkbox" name={pre2016_checkbox_name} onChange={this.togglePreExams} checked={this.state.is_pre_2016} value={this.state.is_pre_2016} />
					<label className="is-pre" htmlFor={'is_pre_'+p.exam[0]}>I took the {p.exam[0]} before 2016</label>
				</div>
			);
		}
		...
		for (var i = 0; i < parseInt(p.fields.length); i++) {
			...
			if( curr_type === 'number' ){
				if( this.state.is_pre_2016 ) {
					eFields.push( <div className={p.fields[i].includes('pre2016') ? 'left exam-field' : hidden_div_classes } key={i}>
									<label htmlFor={curr_name.toLowerCase()}>{display_label+':'}</label>
									<input id={curr_name.toLowerCase()} name={input_name.toLowerCase()} placeholder={placeholder}
									...
									{ validated != null && !validated && <small>Only values between {curr_min} and {curr_max} accepted</small> }
								</div> );
				}else {
					eFields.push( <div className={p.fields[i].includes('pre2016') ? hidden_div_classes : 'left exam-field' } key={i}>
									<label htmlFor={curr_name.toLowerCase()}>{display_label+':'}</label>
									<input id={curr_name.toLowerCase()} name={input_name.toLowerCase()} placeholder={placeholder}
											type={curr_type} min={curr_min} max={curr_max} className={field_class}
											step={p.step} onChange={p.changed} value={score} disabled={p.fields[i].includes('pre2016')} />
				...
				eFields.push( <div className="left exam-field" key={i}>
								<label htmlFor={curr_name.toLowerCase()}>{p.fields[i]+':'}</label>
								<input id={curr_name.toLowerCase()} name={input_name.toLowerCase()} placeholder={placeholder}
...
var GPA_Conversion = React.createClass({
	getInitialState: function(){
		return {
	...
	buildOptionsUI: function(data){
		...
		ui.push( <option key={-1} value={''}>{'Select country...'}</option> );
		_.each(this.state.country_list, function(name, i){
			ui.push( <option key={i} value={name}>{name}</option> );
		});
	...
	buildConversionsUI: function(data){
		...
		_.each(data, function(obj, i){
			ui.push( <Conversion key={i} data={obj} /> );
		});
	...
	render: function(){
		return (
			<div className="grade-conversion hide-for-small-only">
				<div>Grade Conversion</div>
				<label htmlFor="country_conversion">Country</label>
				<select id="country_conversion" name="countryconversion" value={this.state.country} onChange={this.update}>
				...
				<div className="conversions-container stylish-scrollbar-mini">
					{this.state.conversions_ui}
				</div>
			</div>
		);
...
var Conversion = React.createClass({
	render: function(){
		return (
			<div className="conversions">
				<div>{this.props.data.scale}</div>
				<div>{this.props.data.description}</div>
				<div>{this.props.data.us_grade}</div>
			</div>
		);
	}
});
...
ReactDOM.render( <GetStarted_Step5_Component />, document.getElementById('get_started_step5') );