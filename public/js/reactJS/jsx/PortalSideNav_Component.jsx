...
var PortalSideNav_Component = React.createClass({
	...
	buildTabUI: function(){
		...
		_.each(copy, function(obj, i){
			ui.push( <Nav_Item key={i} tab={obj} /> );
		});
...
var Nav_Item = React.createClass({
	getInitialState: function(){
		return {
			this_tab: null  
		};
	...
	render: function(){
		...
		return (
			<li className={item_classes} onClick={tab.handler} data-name={tab.name} onMouseEnter={this.showTip} onMouseLeave={this.hideTip}>
				<div className={classes}>
					{ +tab.count > 0 ? <div className="count text-center">{count}</div> : null}
				</div>
				{ tab.open ? <div className="name">{tab.txt}</div> : null }
				{ this.state.this_tab.showTip ? <div className="tip"><span className="pointer"></span>{tab.txt}</div> : null }
			</li>
		);
	}
});

ReactDOM.render( <PortalSideNav_Component />, document.getElementById('portal-nav-window') );