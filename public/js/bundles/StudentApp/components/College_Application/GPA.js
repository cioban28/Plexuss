...
import { GPA_FIELDS, CONVERSION_GPA_FIELDS  } from './constants'
import { updateProfile, saveApplication, resetSaved, getGPAGradingScales, convertToUnitedStatesGPA, clearChangedFields } from './../../actions/Profile'
...
class GPA extends React.Component{
	...
	componentWillMount(){
		...
		// On mount, send route.id and reset gpa.
	...
	componentDidMount(){
		...
		if ( _profile && _profile.country_id != null && _profile.gpa_applicant_country == null ) {
			dispatch( updateProfile({ gpa_applicant_country: _profile.country_id }) );
		}
	}
	...
	componentWillReceiveProps(np){
		...
		if ( ( (!_profile && np._profile) || (_profile.country_id !== np._profile.country_id) ) && np._profile.country_id != null ) {
			dispatch( updateProfile({ gpa_applicant_country: np._profile.country_id }) );
		}

		if ( np._profile.gpa_applicant_country !== _profile.gpa_applicant_country && np._profile.gpa_applicant_country != null ) {
			dispatch( getGPAGradingScales(np._profile.gpa_applicant_country) );
		}
		...
		if ( np._profile.country_grading_scales !== _profile.country_grading_scales && np._profile.country_grading_scales != null ) {
			if ( np._profile.country_grading_scales.length == 1 ) {
				dispatch( updateProfile({ gpa_applicant_scale: np._profile.country_grading_scales[0].id }) );
			}
		}
		...
		if ( np._profile.gpa_applicant_scale && np._profile.gpa_applicant_scale != _profile.gpa_applicant_scale ) {
			...
			dispatch( updateProfile({ selected_grading_scale: grading_scale, gpa_applicant_value: null, gpa_applicant_value_valid: false }))
		}
		...
		if ( ( Number.isFinite(parseFloat(np._profile.gpa_applicant_value)) || this._getNodeName('gpa_applicant_value') == 'select' ) && np._profile.gpa_applicant_value !== _profile.gpa_applicant_value ) {
			if ( np._profile.selected_grading_scale != null && np._profile.gpa_applicant_value !== '' ) {
				...
				if (np._profile.gpa_applicant_value_valid) {
					dispatch( updateProfile({ grading_conversion_pending: true }));
					this.setTimer = setTimeout(() => {
						dispatch( convertToUnitedStatesGPA(gch_id, old_value, conversion_type) );
					}, 500)
				} else {
					dispatch( updateProfile({ grading_conversion_pending: false }));
				}
			}
		}
	}
	...
	_saveAndContinue(e){
		...
		if( _profile[PAGE_DONE] ){
			...
			dispatch( saveApplication(form, 'gpa', _profile.oneApp_step) );
		}
	}
	...
	_getField(field, readOnly){
		...
		switch (field_type) {
			case 'text':
				return <TextField field={field} readOnly={readOnly} _profile={_profile} dispatch={dispatch} />;
			case 'select':
				return <SelectField field={field} _profile={_profile} dispatch={dispatch} />;
			case 'number':
				return <NumberField field={field} readOnly={readOnly} disableFocus={true} _profile={_profile} dispatch={dispatch} />;
		...
		return (
			<DocumentTitle title={"Plexuss | College Application | "+route.name}>
				<div className="application-container ">
					...
					<form onSubmit={ this._saveAndContinue } className="" noValidate>
						<div className="app-form-container">
							<div className="page-head head-line-height">Grade Point Average (GPA)</div>

							<div className="head-line-height">In order to qualify to apply to a university in the United States, it is critical that we have the right GPA. Please enter your current GPA based on your county's grading scale; we will then convert it to a US GPA.</div>
							<br />
