...
class NumberField extends React.Component {
	...
	_validateNumber(val){
		let { dispatch, field, readOnly, _profile } = this.props,
			name = field.name,
			valid = false;
	...
	componentWillReceiveProps(newProps){
		let { _profile, field, dispatch, set_focus, readOnly } = this.props;
		...
		if (!readOnly && newProps._profile[field.name] == null && newProps._profile[field.name] != _profile[field.name]){
			dispatch( updateProfile({ [field.name+'_valid']: false }) );
		}
	}
