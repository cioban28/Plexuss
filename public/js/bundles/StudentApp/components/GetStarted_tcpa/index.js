...
import { spinjs_config_white } from './../../utils/spinjs_config'
...
class GetStartedTCPA extends Component{
    ...
    render() {
		...
		return (
			<div>
				{!!_user.init_done === false && <div className="spin-wrap"><ReactSpinner config={spinjs_config_white} /></div> }
				{!!_user.init_done === true &&
					<div className="small-12 columns main-container">
						...
						<div className="next-container" onClick={this._save}>
							<div className={"next-btn"}>{!!_user.save_tcpa_data_pending === true ? <div className="spin-wrap spin-wrap2"><ReactSpinner config={spinjs_config_white} /></div> : 'Finish!' }</div>
						</div>
					</div>
				}
			</div>
...
function TCPACard(props) {
	const { college, handleCheckbox, answers } = props;
