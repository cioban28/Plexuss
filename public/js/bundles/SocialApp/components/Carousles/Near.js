...
import { getNearColleges } from './../../api/carousles'
...
class Near extends Component{
    ...
    getDatas(page) {
        getNearColleges(page)
    }

    next = (currentSlide, slideCount) => {
        if (currentSlide + 9 > slideCount 
            && this.props.carousles.loading.near
            && this.props.carousles.status.near)
        {
            this.getDatas(this.props.carousles.start.near)
        }
    ...
    componentDidMount(){
        if (this.props.carousles.datas.near.length <= 0)
            this.getDatas(this.props.carousles.start.near)
        ...
        return (
            <span>
                <div className="title_college" ref={(c) => this.college = c }>
                    <div className="college_title" style={divStyle}>Colleges near you</div>
                </div>
                {this.props.carousles.datas.near.length > 0 ? (
                    <div className="colleges">
                        <Slider ref={c => (this.slider = c)} {...settings}>
                            {Object.entries(this.props.carousles.datas.near).map((data, index) => <Colleges key={index} data={data}/>)}
                        </Slider>
                    </div>
                ) : (
                    <span>{!this.props.carousles.loading.near ? null :
...
export default connect(mapStateToProps, mapDispatchToProps)(Near);
