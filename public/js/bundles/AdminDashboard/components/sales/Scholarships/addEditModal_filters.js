...
import {loadtabData, loadtabData_sc, checkSelect, checkchkbox, onChangeFunc, getCity,getScholarshipData} from './../../../actions/pickACollegeActions'
import $ from "jquery";
...
export default class AddEditModal_Step2 extends React.Component{
	...
	onChangeMajor(e) {
		...
		//let id = e.target.dataset.department-id;
	...
	onChangetab(tabname,data){
		loadtabData(tabname,data);
	}
	...
	onSelectmajor(e){
		...
		/*$("#chk_"+did+" input:checked").each(function() {
			degreelevel.push($(this).val());
		});*/
		
		/*var degreelevel = [];
		$('#mainchkdiv_'+did+' input[type="checkbox"]:checked').each(function() {
			degreelevel.push(this.value); 
		});
		*/
	...
	onSelectsubmajor(e){
		...
		/*var degreelevel = [];
		$("input[name='subchk_"+pid+"[]']:checked").each(function(){degreelevel.push($(this).val());});*/
		
		/*$('#mainchkdiv_'+pid+' input[type="checkbox"]:checked').each(function() {
			degreelevel.push(this.value); 
		});*/
	...
	handledel_majorsub_Item(v){
		...
		if(majorsub_filter_array.length >0){
			...
			data[commentIndex].degreelevel = [ "1", "2", "3", "4", "5"];
			this.setState(
				this.state
			)
			this.state
	...
	render(){
		...
		let {financial_val,interested_in_aid,interested_school_type, profileCompletion,militaryAffiliation,college_users,hs_users,educationLevel,uploadLevel,transcript_filter,financialInfo_filter,ielts_fitler,toefl_filter,resume_filter,passport_filter,essay_filter,other_filter,startDateLevel,countryLevel,stateLevel,cityLevel,statesubLevel,citysubLevel,countryDs,stateDs,cityDs,ageMin_filter_n,ageMax_filter_n,eth_filterVal,rgs_filterVal,eth_filterSel,rgs_filterSel,gpaMinLevel,gpaMaxLevel,satMinLevel,satMaxLevel,actMinLevel,actMaxLevel,toeflMinLevel,toeflMaxLevel,ieltsMinLevel,ieltsMaxLevel,majorDeptVal,chk1,chk2,chk3,chk4,chk5} = '';
						...
						<div className="adv-filtering-menu-container">
							<ul className="side-nav adv-filtering-menu">
								<li data-filter-tab="location" id="location" className="">
									<a onClick={() => {this.onChangetab('location',data)}} className="litext">Location</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="startDateTerm" id="startDateTerm" className="">
									<a onClick={() => {this.onChangetab('startDateTerm',data)}} className="litext">Start Date</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="financial" id="financial" className="">
									<a onClick={() => {this.onChangetab('financial',data)}} className="litext">Financials</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="typeofschool" id="typeofschool" className="">
									<a onClick={() => {this.onChangetab('typeofschool',data)}} className="litext">Type of School</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="majorDeptDegree" id="majorDeptDegree" className="">
									<a onClick={() => {this.onChangetab('majorDeptDegree',data)}} className="litext">Major</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="scores" id="scores" className="">
									<a onClick={() => {this.onChangetab('scores',data)}} className="litext">Scores</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="uploads" id="uploads" className="">
									<a onClick={() => {this.onChangetab('uploads',data)}} className="litext">Uploads</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="demographic" id="demographic" className="">
									<a onClick={() => {this.onChangetab('demographic',data)}} className="litext">Demographic</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="educationLevel" id="educationLevel" className="">
									<a onClick={() => {this.onChangetab('educationLevel',data)}} className="litext">Education Level</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="militaryAffiliation" id="militaryAffiliation" className="">
									<a onClick={() => {this.onChangetab('militaryAffiliation',data)}} className="litext">Military Affiliation</a>
									<div className="change-icon hide"></div>
								</li>
								<li data-filter-tab="profileCompletion" id="profileCompletion" className="">
									<a onClick={() => {this.onChangetab('profileCompletion',data)}} className="litext">Profile Completion</a>
									<div className="change-icon hide"></div>
								</li>
							</ul>
						</div>
											...
											<div className="row contains-tags-row component clearfix" data-component="state" id="stateMainDiv" style={{display:stateLevel}}>
												...
												<div className="selection-row" id="stateDiv" style={{display:statesubLevel}}>
													...
													<select className="select-filter filter-this" name="location_state" id="state_filter" onChange={this.onState_change} value={(stateDs!="" && stateDs!='undefined') ? stateDs : 'undefined'}>
														...
														{states.map((item, i) => {
															return <option key={"c"+i} value={item.name}>{item.name}</option>
														})}
													</select>
													<br />
											...
											<div className="column small-12">
												...
												<div className="selection-row" id="majorDeptDiv" style={{display:majorDeptVal}}>
													...
													<select className="select-filter filter-this" name="specificDepartment_filter" id="specificDepartment_filter" onChange={this.onSelectmajor} >
														<option value="">Select...</option>
														{depts.map((item, i) => {
															return <option key={item.id} value={item.name} data-id={item.id}>{item.name}</option>
														})}
													</select>
												</div>
										...
										<div className="row contains-tags-row component" data-component="Ethnicty">
											<div className="column small-12">
												...
												<div className="selection-row" id="ethnicityDiv" style={{display:eth_filterVal}}>
													...
													<br />
													{ethnicity_filter_array.map((v) => {
														return <span key={v} className="advFilter-tag" data-tag-id={v} data-type-id="Ethnicty">{v}<span className="remove-tag" onClick={this.handledel_ethnicity_Item.bind(this, v)}> x </span></span>
													})}
												</div>
											</div>
										</div>
