...
export const loadtabData = (val,gdata) => {
	//return function(){
		...
		/*if(document.getElementById("isedit").innerHTML==0){
		...
		*/
		...
		if(document.getElementById("isedit").innerHTML==0 || document.getElementById("isedit").innerHTML==2){
			...
			//if(val=="location" || val=="startDateTerm" || val=="financial" || val=="typeofschool" || val=="majorDeptDegree" || val=="militaryAffiliation" || val=="profileCompletion"){
			...
			/*}else{
				...
				var resultElement = document.getElementById('defaultDiv');
				resultElement.innerHTML = '';
				axios.post('/ajax/getScholarshipTargeting', {input: val ,gdata: gdata})
				.then((response) => {
					resultElement.innerHTML = response.data;
